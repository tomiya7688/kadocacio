...
from settings import HOME_RED, TACTIC_NAMES, TEAMS_DIR, clamp, grid_role, grid_slot, parse_hex_color
...
def team_choice_from_payload(payload: dict, source_name: str = "<memory>") -> dict:
    ...
    try:
        ...
        return {
            ...
            "manager_tactic_aggression": manager_stat(info.get("戦術変更への積極性", 500), 500),
            "manager_substitution_aggression": manager_stat(info.get("選手交代への積極性", 500), 500),
            "manager_intelligence": manager_stat(info.get("インテリジェンス", 700), 700),
