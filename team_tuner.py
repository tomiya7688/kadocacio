...
from player_style_system import STYLE_FIELDS, TYPE_PREFERENCES
from settings import FIELD, MATCH_SECONDS, clamp
from simulation_runtime import advance_match_fixed
from team_data import team_choice_from_payload
from team_rating import category_average, rank_for_average
...
def _number(value: object, default: float = 700.0) -> float:
    try:
        return float(value)
...
def _normalize_cells(cells: list[tuple[dict, str]], target: float) -> None:
    if not cells:
        return
    values = [clamp(_number(player.get(field), target), 50.0, 1250.0) for player, field in cells]
    for _ in range(40):
        difference = target - sum(values) / len(values)
        if abs(difference) < 0.35:
            break
        values = [clamp(value + difference, 50.0, 1250.0) for value in values]
...
def auto_adjust_payload(
    ...
    *,
    ...
    for category in categories:
        ...
        target = clamp(float(targets.get(category_id, 700)), 50.0, 1250.0)
        cells: list[tuple[dict, str]] = []
        for player in selected:
            for field in category.get("fields", ()):
                factor = _field_factor(player, str(field))
                jitter = rng.gauss(0.0, target * (0.018 + mutation))
                value = clamp(target * factor + jitter, 50.0, 1250.0)
    ...
    *,
    ...
    for category in categories:
        if category.get("hidden") and not include_hidden:
            continue
        category_id = str(category.get("id"))
        baseline = float(baseline_means.get(category_id, current_means.get(category_id, 700.0)))
...
def mutate_payload_distribution(
    ...
    *,
    ...
    The tuner is allowed to discover uneven allocations such as shoot power 1100
    and shoot accuracy 700.  Only the category-wide mean is a constraint; player
    ...
    for category in categories:
        ...
        for _ in range(transfer_count):
            ...
            transferable = min(1250 - receiver_value, donor_value - 50)
            if transferable <= 0:
                continue
            typical = max(4.0, category_mean * strength * 0.55)
...
class TeamTunerSession:
    ...
    def __init__(
        ...
        *,
        ...
        self.parallel_worker_count = max(1, min(requested_workers, int(max_workers), cpu_workers))
    ...
    def status_text(self) -> str:
        ...
        if not metrics:
            if self.parallel_enabled:
                ...
                return f"分野平均±10固定　{end_label}{self.parallel_worker_count}コアで候補1件を評価中（{finished}/{fixtures}試合終了）"
            return f"分野平均±10固定　{end_label}{self.current_opponent_name} 戦を計算中"
        mode_label = (
            "精度重視"
        ...
        return "分野平均±10固定　" + end_label + prefix + (
