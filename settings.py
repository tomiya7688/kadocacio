...
FIELD_SCALE = 2.4
FIELD = pygame.Rect(34, 72, 2200, 1420)
PANEL = pygame.Rect(954, 20, 306, 680)
GOAL_HALF_HEIGHT = 76
...
PLAYER_STAT_MIN = 50.0
PLAYER_STAT_MAX = 1250.0
...
def player_stat(raw: dict, key: str, default: float = 700.0, legacy_key: str = "Kick") -> float:
    """Read a 50..1250 player stat and return its normalized 0..1 value."""
    value = raw.get(key, raw.get(legacy_key, default))
    try:
        clamped = clamp(float(value), PLAYER_STAT_MIN, PLAYER_STAT_MAX)
        return (clamped - PLAYER_STAT_MIN) / (PLAYER_STAT_MAX - PLAYER_STAT_MIN)
    except (TypeError, ValueError):
        clamped = clamp(default, PLAYER_STAT_MIN, PLAYER_STAT_MAX)
        return (clamped - PLAYER_STAT_MIN) / (PLAYER_STAT_MAX - PLAYER_STAT_MIN)
