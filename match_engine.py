...
from player_commands import PlayerCommand, dribble_movement_speed, movement_speed
...
from prediction_system import expected_goals, outcome_probabilities, percentage_triplet
...
from stamina_system import command_stamina_cost
from team_data import discover_team_choices
from technique_system import (
    choose_trap_style,
    interception_reach,
    trap_success_chance,
    trap_touch_offset,
...
class PendingKick:
    ...
    target_point: pygame.Vector2 | None = None
...
class Match:
    def __init__(
        ...
        self.throw_in_spot = pygame.Vector2(FIELD.center)
        self.thrower: Player | None = None
        self.restart_type = ""
        self.restart_team: Team | None = None
        self.restart_spot = pygame.Vector2(FIELD.center)
        self.restart_taker: Player | None = None
        self.restart_approach = pygame.Vector2(FIELD.center)
        self.restart_elapsed = 0.0
        self.set_piece_targets: dict[Player, pygame.Vector2] = {}
        ...
        self.referee_pos = pygame.Vector2(FIELD.centerx, FIELD.centery + 100)
        self.referee_target = pygame.Vector2(FIELD.center)
        ...
        self.possession_anchor = pygame.Vector2(FIELD.center)
    ...
    def change_owner(
        self,
        player: Player,
        control_offset: pygame.Vector2 | tuple[float, float] | None = None,
    ) -> None:
    ...
    def begin_kick(
        ...
        command: PlayerCommand,
        target_player: Player | None = None,
        target_point: pygame.Vector2 | tuple[float, float] | None = None,
        set_piece_skill: float = 0.0,
        ...
        self.pending_kick = PendingKick(
            owner, command, target_player, set_piece_skill, pygame.Vector2(target_point)
        )
        self.ball.vel.update(0, 0)
    ...
    def start_offside_free_kick(self, owner: Player, target: Player) -> None:
        ...
        self.start_set_piece("FREE_KICK", defending_team, pygame.Vector2(target.pos))
    ...
    def shape_position(self, player: Player, has_possession: bool) -> pygame.Vector2:
        cache = getattr(self, "_shape_position_cache", None)
        cache_key = (player, bool(has_possession))
        if cache is not None and cache_key in cache:
            return pygame.Vector2(cache[cache_key])
        team = player.team
        ...
        result = pygame.Vector2(clamp(base_x, FIELD.left + 20, FIELD.right - 20), clamp(y, FIELD.top + 18, FIELD.bottom - 18))
        if cache is not None:
            cache[cache_key] = pygame.Vector2(result)
        return result

    @staticmethod
    def distance_to_pass_lane(point: pygame.Vector2, start: pygame.Vector2, end: pygame.Vector2) -> float:
        lane = end - start
    ...
    def goal_event_urgency(
        self,
        team: Team,
        point: pygame.Vector2 | None = None,
        ...
        point = pygame.Vector2(point if point is not None else self.ball.pos)
    ...
    def point_in_own_penalty_area(
        self,
        team: Team,
        point: pygame.Vector2 | tuple[float, float],
        margin: float = 0.0,
    ) -> bool:
    ...
    def clamp_to_goalkeeper_area(
        ...
        return pygame.Vector2(clamp(point[0], left, right), clamp(point[1], top, bottom))
    ...
    def cached_off_ball_decision(self, player: Player, context: tuple, chooser) -> tuple:
        ...
        if player.ai_rethink_timer > 0.0 and same_owner:
            return (
                ...
                pygame.Vector2(player.ai_cached_target),
        ...
        player.ai_rethink_timer = 0.12 + (1.0 - player.effective_intelligence) * 0.16
        return action, pygame.Vector2(target), locomotion
    ...
    def support_location_value(self, player: Player, owner: Player, target: pygame.Vector2) -> tuple[float, float]:
    ...
    def spread_attacking_target(
        ...
        for teammate in player.team.players:
            ...
            if distance < minimum:
                if distance < 0.01:
                    direction = pygame.Vector2(0, -1 if (player.grid_x + support_rank) % 2 else 1)
                else:
    ...
    def open_space_target(self, player: Player, origin: pygame.Vector2, forward: float = 100.0) -> pygame.Vector2:
        opponents = [
            opponent for team in self.teams if team is not player.team
            for opponent in team.players if not opponent.sent_off
        ]
    ...
    def defensive_intercept_target(self, player: Player, owner: Player) -> pygame.Vector2:
    ...
    def shot_lane_quality(self, owner: Player, opponents: list[Player]) -> float:
        ...
        for offset in (-0.62, 0.0, 0.62):
            target = pygame.Vector2(goal_x, FIELD.centery + GOAL_HALF_HEIGHT * offset)
            clearance = min(
    ...
    def choose_attacking_run(
        self,
        player: Player,
        shape: pygame.Vector2,
        owner: Player,
    ) -> tuple[PlayerCommand, pygame.Vector2, PlayerCommand]:
        intelligence = player.effective_intelligence
        conservation = player.stamina_conservation
        ...
        support_target = owner.pos + pygame.Vector2(
        ...
        triangle_target = owner.pos + pygame.Vector2(
        ...
        if (
            ...
            behind_marker = marker.pos + pygame.Vector2(player.team.direction * (38 + player.goal_poaching * 48), 0)
        ...
        if command is PlayerCommand.SUPPORT:
            ...
            target = shape + escape * (48 + qualities[command] * 58) + pygame.Vector2(player.team.direction * 34, 0)
            ...
            target = player.pos + pygame.Vector2(player.team.direction * 112, side * -104)
            ...
            target = pygame.Vector2(goal_x - player.team.direction * (76 + (1.0 - qualities[command]) * 58), FIELD.centery + side * 72)
    ...
    def choose_defensive_action(
        ...
        if player.can_consider_skill(RECOVERY_CHASE) and owner_has_passed and distance < 145:
            ...
            if self.try_activate_player_skill(player, RECOVERY_CHASE, 0.48 + (145 - distance) / 330.0, chase_execution, duration=1.15):
                ...
                return PlayerCommand.SKILL_RECOVERY_CHASE, pygame.Vector2(owner.pos), PlayerCommand.DASH
        ...
        if player.can_consider_skill(ONE_WAY_BLOCK) and is_presser and distance < 112:
            ...
            if self.try_activate_player_skill(
                ...
                cut_target = owner.pos + pygame.Vector2(owner.team.direction * 24, cut_side * 28)
        ...
        own_goal = pygame.Vector2(FIELD.left if player.team.direction == 1 else FIELD.right, FIELD.centery)
    ...
    def choose_owner_carry_skill(
    ...
    ) -> tuple[PlayerCommand, pygame.Vector2, PlayerCommand] | None:
        ...
        if player.post_play_timer > 0.0:
            target = player.pos + pygame.Vector2(-player.team.direction * 26, side * 18)
            return PlayerCommand.SKILL_PIVOT_KEEP, target, PlayerCommand.WALK
        if player.razor_dribble_timer > 0.0:
            target = player.pos + pygame.Vector2(player.team.direction * 76, side * 22)
        ...
        if player.toughness_dynamo_timer > 0.0:
            target = player.pos + pygame.Vector2(player.team.direction * 82, side * 20)
        ...
        if selected == ILLUSION_STEP:
            ...
            player.tricky_feint_direction = safe_normalize(
                pygame.Vector2(player.team.direction, side * 0.78) + escape_side * 0.72
            )
            player.tricky_feint_timer = 0.82
        elif selected == HEEL_REVERSE_TURN:
            escape_side = safe_normalize(player.pos - nearest_opponent.pos)
            player.cruyff_direction = safe_normalize(escape_side + pygame.Vector2(-player.team.direction * 0.85, side * 0.45))
    ...
    def update_player_movement(self, dt: float) -> None:
        ...
        for team in self.teams:
            ...
            intercept_targets: dict[Player, pygame.Vector2] = {}
            ...
            for player in team.players:
                if player.sent_off:
                    ...
                    continue
                previous_position = pygame.Vector2(player.pos)
                ...
                if player.knockback_timer > 0.0:
                    ...
                    goal_center = pygame.Vector2(goal_x, FIELD.centery)
                    patrol_depth = clamp(
                    ...
                    if (
                        ...
                        if self.try_activate_player_skill(
                            ...
                            if abs(lateral_gap) < 7:
                                side = 1.0 if player.pos.y < FIELD.centery else -1.0
                            roulette_direction = pygame.Vector2(team.direction, side * 0.62).normalize()
                    ...
                    if carry_skill is not None:
                        ...
                        forward = pygame.Vector2(team.direction, 0) * (28 + player.roulette_success * 52)
                        lateral = pygame.Vector2(0, player.roulette_direction.y) * math.sin(math.pi * phase) * 54
                    ...
                    reception_point = pygame.Vector2(self.ball.intended_destination)
    ...
    def resolve_player_collisions(self) -> None:
        ...
        for index, first in enumerate(players):
            for second in players[index + 1:]:
                ...
                normal = safe_normalize(delta) if distance > 0.01 else pygame.Vector2(0, 1)
    ...
    def consider_aerial_jumps(self) -> None:
        ...
        for team in self.teams:
            ...
            for player in candidates:
                ...
                predicted_position = true_position + pygame.Vector2(
                ...
                predicted_position += pygame.Vector2(
    ...
    def header_ball(self, player: Player) -> None:
        ...
        if purpose in (PlayerCommand.SKILL_AERIAL_HEADER, PlayerCommand.SKILL_OVERHEAD_VOLLEY):
            ...
            if self.rng.random() > 0.48 + decision * 0.47:
                open_side *= -1.0
            target = pygame.Vector2(goal_x, FIELD.centery + open_side * GOAL_HALF_HEIGHT * (0.30 + decision * 0.42))
    ...
    def foul_is_penalty(self, defending_team: Team, spot: pygame.Vector2) -> bool:
    ...
    def predicted_pass_destination(
        ...
        visible_velocity = pygame.Vector2(candidate.motion_velocity)
    ...
    def evaluate_pass_route(self, owner: Player, candidate: Player) -> PassRoute:
        ...
        destination = pygame.Vector2(candidate.pos)
        flight_time = 0.5
    ...
    def _release_keeper_throw(self, keeper: Player, target: Player) -> None:
        ...
        lead = pygame.Vector2(keeper.team.direction * min(42.0, distance * 0.12), 0)
    ...
    def _release_pass(
        ...
        target: Player,
        set_piece_skill: float = 0.0,
        planned_destination: pygame.Vector2 | None = None,
    ) -> None:
        ...
        destination = pygame.Vector2(planned_destination) if planned_destination is not None else route.destination
        distance = owner.pos.distance_to(destination)
    ...
    def clear_destination(self, owner: Player, target: Player | None) -> pygame.Vector2:
    ...
    def _release_shot(self, owner: Player, set_piece_skill: float = 0.0) -> None:
        ...
        direction = safe_normalize(pygame.Vector2(aim_x, target_y) - owner.pos)
        self.ball.owner = None
        self.ball.flight_serial += 1
        self.ball.last_touch = owner
        self.ball.intended = None
    ...
    def update_owner_decision(self, dt: float) -> None:
        ...
        if owner is None:
            return
        normal_offset = pygame.Vector2(owner.team.direction * 10, 0)
        if owner.cruyff_timer > 0.0:
            ...
            tight_offset = pygame.Vector2(owner.team.direction * 4.2, 0)
        ...
        if owner.is_keeper and self.ball.last_keeper_response == "CATCH":
            held_offset = pygame.Vector2(owner.team.direction * 2.5, 0)
    ...
    def resolve_keeper_contact(self, keeper: Player, incoming_speed: float, guardian: bool = False) -> bool:
        ...
        if response is PlayerCommand.PUNCH:
            ...
            if counter_target is not None:
                ...
                outward = pygame.Vector2(keeper.team.direction, clamp(goal_y_delta / 95.0, -0.78, 0.78))
    ...
    def configure_set_piece_positions(
        ...
        for team in self.teams:
            for player in team.players:
                ...
                self.set_piece_targets[player] = pygame.Vector2(target)
        ...
        approach = pygame.Vector2(-attacking.direction * approach_distance, 0)
        if restart_type == "CORNER_KICK":
            approach.y = 12 if spot.y <= FIELD.top else -12
        self.restart_approach.update(spot + approach)
        self.set_piece_targets[taker] = pygame.Vector2(self.restart_approach)
        ...
        if restart_type == "FREE_KICK":
            ...
            for index, player in enumerate(wall_candidates[:wall_size]):
                self.set_piece_targets[player] = pygame.Vector2(
            ...
            self.set_piece_targets[defending.keeper] = pygame.Vector2(
    ...
    def move_set_piece_players(self, dt: float) -> float:
        ...
        for player, target in self.set_piece_targets.items():
            if player.sent_off:
                continue
            active += 1
            previous = pygame.Vector2(player.pos)
    ...
    def start_set_piece(self, restart_type: str, team: Team, spot: pygame.Vector2) -> None:
    ...
    def start_throw_in(self) -> None:
        ...
        inside = pygame.Vector2(self.throw_in_spot)
        if spot_y == FIELD.top:
            inside.y += 13
        ...
        self.set_piece_targets[self.thrower] = pygame.Vector2(inside)
        receivers = [
    ...
    def update_throw_in(self, dt: float) -> None:
        ...
        destination = target.pos + pygame.Vector2(thrower.team.direction * 12, 0)
    ...
    def update_loose_ball(self, dt: float) -> None:
        ...
        if self.ball.knuckle_amplitude > 0.1 and self.ball.vel.length_squared() > 0.01:
            ...
            sideways = pygame.Vector2(-travel.y, travel.x)
        ...
        for player in nearby:
            ...
            if player.pos.distance_to(self.ball.pos) <= radius:
                ...
                if style is not None:
                    ...
                    touch_offset = pygame.Vector2(trap_touch_offset(style, player.team.direction, self.ball.vel.y))
                    player.last_trap_style = style.value
