"""Runtime state and command execution for one football player."""
...
class Player:
    def __init__(
        ...
        for behavior, field in STYLE_FIELDS.items():
            default = denormalize_player_stat(type_preference(self.player_type, behavior))
        ...
        self.motion_velocity = Vec2()
        self.collision_cooldown = 0.0
        self.knockback_timer = 0.0
        self.knockback_velocity = Vec2()
        ...
        self.slide_direction = Vec2(self.team.direction, 0)
        ...
        self.tricky_feint_direction = Vec2(self.team.direction, 0)
    ...
    def start_slide(self, target: Vec2 | tuple[float, float]) -> bool:
    ...
    def start_knockback(self, direction: Vec2, speed: float) -> None:
        self.knockback_timer = 0.42
        self.knockback_velocity = Vec2(direction) * max(45.0, speed)
    ...
    def start_roulette(self, direction: Vec2, success: float) -> None:
        self.roulette_timer = 0.68
        self.roulette_direction = Vec2(direction)
        self.roulette_success = max(0.0, min(1.0, success))

    def start_heading_motion(self, diving: bool, target: Vec2 | tuple[float, float]) -> None:
        direction = Vec2(target) - self.pos
        if direction.length_squared() < 0.0001:
            direction.update(self.team.direction, 0)
        else:
    ...
    def issue_action(
        ...
        target: Vec2 | tuple[float, float] | None = None,
