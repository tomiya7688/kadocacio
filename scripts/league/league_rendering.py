...
from scripts.league.league_auto_progress import WATCH_FOCUS, WATCH_MODE_LABELS, WATCH_MODES
...
from scripts.core.settings import CREAM, GOLD, HEIGHT, HOME_RED, INK, MUTED, PAPER, SPEED_OPTIONS, WIDTH
...
class LeagueRendererMixin:
    ...
    def draw_league_screen(self) -> None:
        ...
        editor_only = bool(getattr(self, "league_editor_only", False))
        self.text("リーグ戦エディタ" if editor_only else "リーグ戦", 30, CREAM, (24, 31), bold=True)
        template_count = max(1, len(getattr(self, "league_template_entries", [])))
        template_number = min(template_count, int(getattr(self, "league_template_index", 0)) + 1)
        header_label = (
            f"{template_number}/{template_count}  {manager.template_name}"
            if editor_only else manager.date_label
        )
        self.text(header_label, 18 if editor_only else 22, CREAM, (WIDTH - 24, 27), bold=True, right=True)
        ...
        if not editor_only:
            ...
            self._league_button(
                pygame.Rect(sidebar.left + 16, sidebar.bottom - 140, sidebar.width - 32, 34),
                "オート進行を停止" if auto_running else "DEBUG オート進行",
                "auto_stop" if auto_running else "auto_open",
                active=auto_running,
                accent=HOME_RED if auto_running else (88, 137, 190),
                small=True,
            )
        self._league_button(
        ...
        if editor_only:
            pygame.draw.rect(self.screen, PAPER, sidebar, border_radius=12)
            pygame.draw.rect(self.screen, GOLD, sidebar, 2, border_radius=12)
            self.text("リーグ戦テンプレート", 16, INK, (sidebar.left + 16, sidebar.top + 18), bold=True)
            self.text("標準は leagues.json、追加分は", 10, MUTED, (sidebar.left + 16, sidebar.top + 50))
            self.text("league_templates/*.json に保存されます。", 10, MUTED, (sidebar.left + 16, sidebar.top + 68))
            selector_y = sidebar.top + 102
            self._league_button(pygame.Rect(sidebar.left + 16, selector_y, 32, 34), "◀", "template_cycle|-1", small=True)
            template_rect = pygame.Rect(sidebar.left + 54, selector_y, 158, 34)
            pygame.draw.rect(self.screen, (255, 253, 242), template_rect, border_radius=7)
            pygame.draw.rect(
                self.screen,
                GOLD if getattr(self, "league_template_input_active", False) else (176, 174, 165),
                template_rect,
                2,
                border_radius=7,
            )
            template_text = self.league_template_name + (self.ime_composition if self.league_template_input_active else "")
            while len(template_text) > 4 and self.font(11, True).size(template_text)[0] > template_rect.width - 16:
                template_text = template_text[:-2] + "…"
            self.text(template_text or "テンプレート名", 11, INK, (template_rect.left + 8, template_rect.top + 9), bold=True)
            self.league_buttons.append((template_rect, "template_input"))
            self._league_button(pygame.Rect(sidebar.left + 218, selector_y, 32, 34), "▶", "template_cycle|1", small=True)
            self._league_button(
                pygame.Rect(sidebar.left + 16, sidebar.top + 148, 110, 34),
                "複製作成", "template_duplicate", accent=(91, 180, 111), small=True,
            )
            self._league_button(
                pygame.Rect(sidebar.left + 134, sidebar.top + 148, 116, 34),
                "名前変更", "template_rename", small=True,
            )
            deleting_template = self.league_template_delete_confirm == manager.template_id
            self._league_button(
                pygame.Rect(sidebar.left + 16, sidebar.top + 190, sidebar.width - 32, 34),
                "削除する" if deleting_template else "テンプレート削除",
                "template_delete", active=deleting_template, accent=HOME_RED, small=True,
            )
            self.text("選択中のテンプレートだけを編集します。", 10, MUTED, (sidebar.left + 16, sidebar.top + 244), bold=True)
            self._league_button(
                pygame.Rect(sidebar.left + 16, sidebar.bottom - 56, sidebar.width - 32, 36),
                "メインメニューへ", "back",
            )
        ...
        if session is not None:
            ...
            auto_running = bool(getattr(self, "league_auto_running", False))
            panel_height = 238 if auto_running else 192
            panel = pygame.Rect(WIDTH // 2 - 270, HEIGHT // 2 - panel_height // 2, 540, panel_height)
            ...
            f"論理{session.detected_cpu_count}CPU・空きメモリ{session.available_memory_gb:.1f}GBを考慮",
                10, MUTED, (panel.centerx, panel.bottom - (70 if auto_running else 25)), center=True,
            )
            if auto_running:
                self._league_button(
                    pygame.Rect(panel.centerx - 116, panel.bottom - 48, 232, 34),
                    "オート進行を停止", "auto_stop", accent=HOME_RED, small=True,
                )
    ...
    def _draw_league_auto_config(self) -> None:
        shade = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        shade.fill((8, 14, 22, 210))
        self.screen.blit(shade, (0, 0))
        card = pygame.Rect(WIDTH // 2 - 390, 66, 780, 588)
        pygame.draw.rect(self.screen, (246, 243, 230), card, border_radius=18)
        pygame.draw.rect(self.screen, (88, 137, 190), card, 3, border_radius=18)
        pygame.draw.rect(self.screen, (88, 137, 190), (card.left, card.top, card.width, 5), border_radius=3)
        self.text("DEBUG TOOL", 10, (62, 107, 157), (card.left + 28, card.top + 22), bold=True)
        self.text("リーグ戦オート進行", 27, INK, (card.left + 28, card.top + 42), bold=True)
        self.text(
            "通常ゲームプレイ用ではありません。開始後は停止するまで次の試合日へ進み続けます。",
            11, MUTED, (card.left + 28, card.top + 82),
        )

        config = self.league_auto_config
        section_left = card.left + 28
        section_width = card.width - 56
        self.text("観戦方法", 14, INK, (section_left, card.top + 120), bold=True)
        mode_gap = 10
        mode_width = (section_width - mode_gap * 2) // 3
        mode_details = {
            "RANDOM": "毎節1試合をランダム表示",
            "NONE": "全試合をヘッドレス高速処理",
            "FOCUS": "指定チームの試合だけ表示",
        }
        for index, mode in enumerate(WATCH_MODES):
            rect = pygame.Rect(section_left + index * (mode_width + mode_gap), card.top + 148, mode_width, 62)
            self._league_button(
                rect,
                WATCH_MODE_LABELS[mode],
                f"auto_watch|{mode}",
                active=config.watch_mode == mode,
                accent=(88, 137, 190),
                small=True,
            )
            self.text(mode_details[mode], 9, MUTED, (rect.centerx, rect.bottom + 8), center=True)

        self.text("観戦試合の速度", 14, INK, (section_left, card.top + 246), bold=True)
        self.text("観戦なしの場合、描画せず可能な限り速く計算します", 10, MUTED, (section_left + 138, card.top + 250))
        speed_gap = 8
        speed_width = (section_width - speed_gap * 5) // 6
        current_speed_index = SPEED_OPTIONS.index(config.speed_multiplier)
        for index, speed in enumerate(SPEED_OPTIONS):
            rect = pygame.Rect(section_left + index * (speed_width + speed_gap), card.top + 276, speed_width, 38)
            self._league_button(
                rect, f"×{speed}",
                f"auto_speed|{index - current_speed_index}",
                active=config.speed_multiplier == speed,
                accent=(88, 137, 190),
                small=True,
            )

        self.text("フォーカスするチーム", 14, INK, (section_left, card.top + 348), bold=True)
        focus_choice = self.current_league_auto_focus_choice()
        focus_name = str(focus_choice.get("name", "チームなし")) if focus_choice else "チームなし"
        self._league_button(
            pygame.Rect(section_left, card.top + 378, 42, 40), "◀", "auto_focus|-1", small=True,
        )
        focus_rect = pygame.Rect(section_left + 50, card.top + 378, section_width - 100, 40)
        focus_active = config.watch_mode == WATCH_FOCUS
        pygame.draw.rect(self.screen, (232, 239, 244) if focus_active else (229, 226, 215), focus_rect, border_radius=7)
        pygame.draw.rect(self.screen, (88, 137, 190) if focus_active else (174, 171, 160), focus_rect, 2, border_radius=7)
        self.text(focus_name, 13, INK, focus_rect.center, bold=True, center=True)
        self._league_button(
            pygame.Rect(focus_rect.right + 8, card.top + 378, 42, 40), "▶", "auto_focus|1", small=True,
        )
        self.text(
            "指定チームの試合がない開催日は、全カードをヘッドレスで処理します。",
            10, MUTED, (section_left, card.top + 432),
        )
        self.text(
            "非観戦試合も通常と同じ Match エンジンを使用し、結果だけの簡易抽選は行いません。",
            10, (62, 107, 157), (section_left, card.top + 462), bold=True,
        )
        self._league_button(
            pygame.Rect(card.left + 28, card.bottom - 62, 190, 40),
            "キャンセル", "auto_cancel", small=True,
        )
        self._league_button(
            pygame.Rect(card.right - 258, card.bottom - 62, 230, 40),
            "この設定で開始", "auto_start", accent=(91, 180, 111),
        )
    ...
    def _draw_league_save_select(self) -> None:
        ...
        self.text("使用するリーグ戦テンプレート", 14, INK, (panel.left + 26, panel.top + 16), bold=True)
        template_y = panel.top + 40
        self._league_button(pygame.Rect(panel.left + 26, template_y, 36, 36), "◀", "template_cycle|-1", small=True)
        template_rect = pygame.Rect(panel.left + 70, template_y, 310, 36)
        pygame.draw.rect(self.screen, (255, 253, 242), template_rect, border_radius=7)
        pygame.draw.rect(
            self.screen,
            GOLD if getattr(self, "league_template_input_active", False) else (176, 174, 165),
        ...
        template_text = self.league_template_name + (self.ime_composition if self.league_template_input_active else "")
        self.text(template_text or "テンプレート名", 12, INK, (template_rect.left + 10, template_rect.top + 9), bold=True)
        self.league_buttons.append((template_rect, "template_input"))
        self._league_button(pygame.Rect(panel.left + 388, template_y, 36, 36), "▶", "template_cycle|1", small=True)
        self._league_button(pygame.Rect(panel.left + 438, template_y, 104, 36), "複製作成", "template_duplicate", accent=(91, 180, 111), small=True)
        self._league_button(pygame.Rect(panel.left + 550, template_y, 104, 36), "名前変更", "template_rename", small=True)
        deleting_template = self.league_template_delete_confirm == manager.template_id
        self._league_button(
            pygame.Rect(panel.left + 662, template_y, 112, 36),
            "削除する" if deleting_template else "削除",
            "template_delete", active=deleting_template, accent=HOME_RED, small=True,
        )
        template_count = max(1, len(getattr(self, "league_template_entries", [])))
        self.text(
            f"{int(getattr(self, 'league_template_index', 0)) + 1} / {template_count}",
            11, MUTED, (panel.left + 790, template_y + 10), bold=True,
        )

        self.text("新しいリーグ戦", 16, INK, (panel.left + 26, panel.top + 88), bold=True)
        input_rect = pygame.Rect(panel.left + 26, panel.top + 112, 420, 38)
        ...
        self._league_button(pygame.Rect(panel.right - 174, input_rect.top, 144, input_rect.height), "メニューへ戻る", "save_back", small=True)
        ...
        for index, info in enumerate(saves[:6]):
            row = pygame.Rect(panel.left + 24, panel.top + 208 + index * 58, panel.width - 48, 50)
            ...
            detail = f"{info.get('template_name', '標準テンプレート')} / year{info['year']}  {info['day']}日目"
    ...
    def _draw_league_schedule(self, main: pygame.Rect) -> None:
        ...
        self.league_schedule_scroll = clamp_schedule_scroll(self.league_schedule_scroll, len(fixtures))
        visible, first_index, last_index = visible_schedule_fixtures(fixtures, self.league_schedule_scroll)
        count_text = f"全{len(fixtures)}試合　表示 {first_index + 1}～{last_index}" if fixtures else "全0試合"
        self.text(count_text, 10, INK, (main.right - 26, main.top + 73), right=True, bold=True)
        self.text("同時開催の全カードから観戦する1試合を選べます（ホイールでスクロール）", 11, MUTED, (main.left + 24, main.top + 96))

        viewport = pygame.Rect(main.left + 20, main.top + 126, main.width - 58, 328)
        self.league_schedule_scroll_rect = viewport.inflate(0, 12)
        column_gap = 8
        row_gap = 4
        row_height = 37
        column_width = (viewport.width - column_gap * (SCHEDULE_COLUMNS - 1)) // SCHEDULE_COLUMNS
        for visible_index, fixture in enumerate(visible):
            column = visible_index % SCHEDULE_COLUMNS
            grid_row = visible_index // SCHEDULE_COLUMNS
            rect = pygame.Rect(
                viewport.left + column * (column_width + column_gap),
                viewport.top + grid_row * (row_height + row_gap),
                column_width,
                row_height,
            )
            source_index = first_index + visible_index
            pygame.draw.rect(self.screen, (235, 232, 219) if source_index % 2 == 0 else (227, 225, 214), rect, border_radius=5)
            pygame.draw.rect(self.screen, (200, 196, 183), rect, 1, border_radius=5)
            fixture_label = fixture.get("competition_label") or f"{fixture['league']} 第{fixture['round']}節"
            watch_rect = pygame.Rect(rect.right - 104, rect.top + 5, 96, 28)
            text_width = watch_rect.left - rect.left - 14
            matchup = f"{fixture['home_name']} vs {fixture['away_name']}"
            while len(matchup) > 5 and self.font(9, True).size(matchup)[0] > text_width:
                matchup = matchup[:-2] + "…"
            label = str(fixture_label)
            while len(label) > 4 and self.font(8).size(label)[0] > text_width:
                label = label[:-2] + "…"
            self.text(label, 8, MUTED, (rect.left + 8, rect.top + 4))
            self.text(matchup, 9, INK, (rect.left + 8, rect.top + 20), bold=True)
            reserved = manager.watch_fixture_id == fixture["id"]
            self._league_button(
                watch_rect,
                "予約中" if reserved else "観戦予約",
                f"watch|{fixture['id']}",
                active=reserved,
                accent=HOME_RED,
                small=True,
        ...
        maximum_scroll = schedule_max_scroll(len(fixtures))
        track = pygame.Rect(viewport.right + 10, viewport.top, 10, viewport.height)
        self.league_schedule_scroll_track = track
        pygame.draw.rect(self.screen, (205, 202, 190), track, border_radius=5)
        if maximum_scroll > 0:
            total_rows = maximum_scroll + SCHEDULE_VISIBLE_ROWS
            thumb_height = max(32, round(track.height * SCHEDULE_VISIBLE_ROWS / total_rows))
            thumb_y = track.top + round((track.height - thumb_height) * self.league_schedule_scroll / maximum_scroll)
            thumb = pygame.Rect(track.left, thumb_y, track.width, thumb_height)
            pygame.draw.rect(self.screen, HOME_RED, thumb, border_radius=5)
            if thumb.top > track.top:
                self.league_buttons.append((pygame.Rect(track.left - 5, track.top, track.width + 10, thumb.top - track.top), f"schedule_scroll|-{SCHEDULE_VISIBLE_ROWS}"))
            if thumb.bottom < track.bottom:
                self.league_buttons.append((pygame.Rect(track.left - 5, thumb.bottom, track.width + 10, track.bottom - thumb.bottom), f"schedule_scroll|{SCHEDULE_VISIBLE_ROWS}"))
        else:
            thumb = track.copy()
            pygame.draw.rect(self.screen, (166, 163, 153), thumb, border_radius=5)
        self.league_schedule_scroll_thumb = thumb
