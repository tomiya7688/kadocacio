...
from settings import TACTIC_NAMES, TEAMS_DIR
from skill_system import ALL_SKILLS, normalize_skill_name
from team_data import PLAYER_KEY_ALIASES
from team_editor_config import load_editor_options
...
def validate_payload(payload: object) -> list[EditorIssue]:
    ...
    for key in ("戦術変更への積極性", "選手交代への積極性", "インテリジェンス"):
        ...
        if key in info and (value is None or not 50 <= value <= 1250):
            issues.append(EditorIssue(f"チーム情報.{key}", "50〜1250で指定してください"))
    ...
    for index, player in enumerate(players):
        ...
        for key in STAT_FIELDS:
            ...
            if value is None or not 50 <= value <= 1250:
                issues.append(EditorIssue(f"{prefix}.{key}", "50〜1250で指定してください"))
...
def _random_stat(rng: random.Random, target: int | None, spread: str) -> int:
    if target is None:
        return rng.randint(50, 1250)
    sigma = {"小": 45, "中": 145, "大": 300}.get(spread, 145)
    return max(50, min(1250, round(rng.gauss(target, sigma))))
...
def create_team_template(
    ...
    *,
    ...
    target = max(50, min(1250, int(target)))
    payload = {
        "選手一覧": [
    ...
    if mode == "target":
        # Clamping near 50/1250 would otherwise bias the average away from the
        ...
        for _ in range(64):
            ...
            for player in payload["選手一覧"]:
                for key in STAT_FIELDS:
                    player[key] = str(max(50, min(1250, round(int(player[key]) + offset))))
    if mode == "random":
        ...
        payload["チーム情報"]["戦術変更への積極性"] = str(rng.randint(50, 1250))
        payload["チーム情報"]["選手交代への積極性"] = str(rng.randint(50, 1250))
        payload["チーム情報"]["インテリジェンス"] = str(rng.randint(50, 1250))
    elif mode == "target":
...
def repair_payload(payload: object, rng: random.Random | None = None) -> dict:
    ...
    for key in ("戦術変更への積極性", "選手交代への積極性", "インテリジェンス"):
        value = _number(info.get(key))
        if value is None or not 50 <= value <= 1250:
            info[key] = str(rng.randint(50, 1250))
    ...
    for index, player in enumerate(players, 1):
        ...
        for key in STAT_FIELDS:
            value = _number(player.get(key))
            if value is None or not 50 <= value <= 1250:
                player[key] = str(rng.randint(50, 1250))
