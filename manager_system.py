...
from settings import PLAYER_STAT_MAX, PLAYER_STAT_MIN, clamp
...
def manager_stat(value: object, default: float) -> float:
    try:
        numeric = clamp(float(value), PLAYER_STAT_MIN, PLAYER_STAT_MAX)
    except (TypeError, ValueError):
        numeric = clamp(default, PLAYER_STAT_MIN, PLAYER_STAT_MAX)
    return (numeric - PLAYER_STAT_MIN) / (PLAYER_STAT_MAX - PLAYER_STAT_MIN)
