...
from simulation_runtime import MatchTelemetry, SimulationLimits, run_headless_match
from team_data import PLAYER_KEY_ALIASES, discover_team_choices


OUTPUT_DIR = Path(__file__).resolve().parent / "ai_evaluation"
...
def _with_parameter(choice: dict, parameter: str, value: int) -> dict:
    ...
    for record in tuple(changed.get("starters", ())) + tuple(changed.get("bench", ())):
        record["raw"][parameter] = str(max(50, min(1250, int(value))))
...
def evaluate(args: argparse.Namespace) -> tuple[dict, Path, Path]:
    ...
    values = [max(50, min(1250, int(part))) for part in args.values.split(",") if part.strip()]
    if not values:
    ...
    if args.processes == "auto":
        process_count = max(1, min(cpu_count - 1 or 1, args.max_processes, len(jobs)))
    else:
        process_count = max(1, min(int(args.processes), args.max_processes, len(jobs)))
    if process_count == 1:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--values", default="50,650,1250", help="比較値。カンマ区切り")
