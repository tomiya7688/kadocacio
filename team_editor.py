...
from player_style_system import PLAYER_TYPES
from settings import CREAM, GOLD, HEIGHT, HOME_RED, INK, MUTED, PAPER, TACTIC_NAMES, TEAMS_DIR, WIDTH
from skill_system import ALL_SKILLS
from team_editor_config import load_editor_options, load_tuner_options
from team_editor_data import (
    FORMATION_TEMPLATES,
    STAT_GROUPS,
    TEAM_FIELDS,
    EditorIssue,
...
from team_rating import category_average, rank_for_average, reload_rating_options
from team_tuner import TeamTunerSession, auto_adjust_payload, payload_category_summary
...
class TeamEditor:
    ...
    def _ensure_tuner_settings(self) -> dict:
        ...
        default_target = int(self.tuner_options.get("default_target", 700))
    ...
    def _tuner_targets(self) -> dict[str, int]:
        ...
        for category in self.tuner_categories:
            ...
            try:
                value = int(values.get(category_id, self.tuner_options.get("default_target", 700)))
            except (TypeError, ValueError):
                value = int(self.tuner_options.get("default_target", 700))
            result[category_id] = max(50, min(1250, value))
    ...
    def refresh_files(self) -> None:
        self.team_entries = scan_team_files()
        self.list_scroll = min(self.list_scroll, max(0, len(self.team_entries) - 8))
    ...
    def _draw_team_list(self) -> None:
        ...
        self.game.text("既存チーム", 18, INK, (left.left + 20, left.top + 16), bold=True)
        self.game.text("不完全なJSONも開いて修正できます", 11, MUTED, (left.left + 20, left.top + 45))

        visible = self.team_entries[self.list_scroll:self.list_scroll + 9]
        for row_index, (path, payload, issues) in enumerate(visible):
            y = left.top + 78 + row_index * 52
            ...
            self._text_fit(name, 14, INK, pygame.Rect(row.left + 10, row.top + 2, 330, 23), bold=True)
            self._text_fit(path.relative_to(TEAMS_DIR).as_posix(), 9, MUTED, pygame.Rect(row.left + 10, row.top + 23, 330, 18))
    ...
    def _draw_tuner(self, content: pygame.Rect) -> None:
        ...
        for index, category in enumerate(self.tuner_categories):
            ...
            if selected is not None:
                try:
                    target_average = float(targets.get(category_id, 700) or 700)
                except (TypeError, ValueError):
                    target_average = float(self.tuner_options.get("default_target", 700))
                secondary_label = (
    ...
    def handle_event(self, event: pygame.event.Event, mouse_pos: tuple[int, int] | None = None) -> None:
        ...
        if event.type == pygame.MOUSEWHEEL:
            if self.mode == "LIST":
                self.list_scroll = max(0, min(max(0, len(self.team_entries) - 9), self.list_scroll - event.y))
    ...
    def _perform(self, action: str, data: Any) -> None:
        if action == "close":
            ...
            valid_count = sum(1 for _, _, issues in self.team_entries if not issues)
            ...
            self.list_scroll = min(max(0, len(self.team_entries) - 9), self.list_scroll + 1)
            ...
            try:
                target = int(self.target_mean or 700)
            except ValueError:
                target = 700
            ...
            try:
                ...
                self._set_message(f"{settings.get('評価モード', '精度重視')}で{len(opponents)}チームを相手に{end_text}、{cores}コアでヘッドレス試合を開始しました", OK_GREEN)
